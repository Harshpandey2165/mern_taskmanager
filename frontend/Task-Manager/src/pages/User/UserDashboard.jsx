...
const UserDashboard = () => {
  ...
  const prepareChartData = (data) => {
    ...
    //  {status :"Cancelled", count: taskDistribution?.Cancelled || 0}, 
    ];
  ...
  const getDashboardData = async () => {
    try {
      const reponse = await axiosInstance.get(API_PATHS.TASKS.GET_USER_DASHBOARD_DATA);
      if(reponse.data){
        setDashboardData(reponse.data);
        prepareChartData(reponse.data?.charts || null);
      }
    } catch (error) {
