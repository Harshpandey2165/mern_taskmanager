...
const ViewTaskDetails = () => {
  ...
  const getStatusTagColor = (status) => {
    switch (status) {
      case 'In Progress':
        return 'bg-cyan-50 text-cyan-50 border border-cyan-500/10'; 
      case 'Completed':
        return 'bg-lime-50 text-lime-50 border border-lime-500/10';
  ...
  const updateTodoChecklist = async(todoId) => {  // Changed parameter to todoId
    const todoChecklist = [...task?.todoChecklist];
    const taskId = id;

    // Find the index of the todo item
    const index = todoChecklist.findIndex(todo => todo._id === todoId);
    
    if(todoChecklist && index !== -1){  // Changed condition
      ...
      try {
        ...
        console.log("Error in updating todo checklist", error);
      }
    }
  }
          ...
          <div className="form-card col-span-3">
            ...
            <div className="col-span-6 md:col-span-4">
              ...
              <div className="mt-2">
                ...
                onChange={() => updateTodoChecklist(item._id)}  // Pass item._id instead of index
                  />
                ))}
              </div>
...
const TodoCheckList = ({text, isChecked, onChange}) => {
  return <div className="flex items-center gap-3 p-3">
    <input type="checkbox" checked={isChecked} onChange={onChange} className="x-4 h-4 text-primary bg-gray-100 border-gray-300 rounded-sm outline-none cursor-pointer" />
