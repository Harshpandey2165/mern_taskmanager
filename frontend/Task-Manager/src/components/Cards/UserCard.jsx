  ...
  return (
    <div className='user-card p-2'>
        ...
        <div className="flex items-end gap-3 mt-5">
            <StatCard label="Pending" count={userInfo?.pendingTasks || 0}
            status = "Pending"
            />
            <StatCard label="In Progress" count={userInfo?.inProgressTasks || 0}
            status = "In Progress"
            />
            <StatCard label="Completed" count={userInfo?.completedTasks || 0}
            status = "Completed"
            />
        </div>
    </div>
...
const StatCard = ({label, count, status}) => {
    ...
    const getStatusTagColor = ()=>{
        switch(status){
            case "Pending": return "text-cyan-500 bg-gray-50";
