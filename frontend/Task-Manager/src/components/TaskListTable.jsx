...
const TaskListTable = ({tableData}) => {
    const getStatusBadgeColor = (status) => {
        switch (status) {
            ...
            default:
                return 'bg-gray-100 text-gray-500 border border-gray-200'; // Default color for unknown stat
        } 
    }
        ...
        <tbody>
            {tableData.map((task, index) => (
                <tr key={index} className="border-t border-gray-200">
                    ...
                    <td className="py-4 px-4 text-gray text-[13px] text-nowrap hidden md:table-cell">{task.dueDate ? moment(task.dueDate).format('Do MMM YYYY') : 'N/A'}</td>
                </tr>
            ))}
            </tbody>
      </table>
